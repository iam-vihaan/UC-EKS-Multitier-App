...
# This module creates all IAM roles and policies required for the EKS cluster
...
# CloudWatch agent policy for nodes
resource "aws_iam_role_policy" "eks_node_cloudwatch" {
  name = "${local.name_prefix}-node-cloudwatch-policy"
...
# IRSA Role for Backend (Secrets Manager access)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:backend:backend-service-account"
...
resource "aws_iam_role_policy_attachment" "backend_secrets_manager" {
